...
//
// Note on fill timer:
//   A player may withdraw regardless of whether the fill timer has expired.
//   Timer expiry does not block withdrawal — it only blocks new deposits.
//   close_stalled_pool.rs handles the state-only cleanup when timer expires
//   but players remain. This instruction handles fund recovery.
//
// PDA seeds:
...
pub fn handler(ctx: Context<Withdraw>) -> Result<()> {
    ...
    //
    // Filling: normal withdrawal during fill phase.
    // Closed: pool was stalled and closed by close_stalled_pool.rs.
    //         Players can still withdraw their stake after the pool is closed.
    require!(
    ...
    //
    // If pool is still Filling, enforce the single-filling-pool invariant.
    // If pool is Closed, active_filling_pool is already None — skip this check.
    ...
    //
    // If the last player withdraws, the pool is empty.
    // We must return rollover_seed to GlobalState and mark pool Closed.
    //
    // Two cases:
    //   a) Pool was still Filling — close it now, clear active_filling_pool.
    //   b) Pool was already Closed (via close_stalled_pool) — active_filling_pool
    //      is already None. Just return rollover seed if present.
    let pool_closed = pool.player_count == 0;

    if pool_closed {
        let rollover_seed = pool.rollover_seed;

        // Return rollover seed to global vault if one was seeded
        if rollover_seed > 0 {
            token::transfer(
                CpiContext::new_with_signer(
                    token_program_ai.clone(),
        ...
        // Only update status and clear pointer if still Filling.
        // If already Closed, close_stalled_pool already handled this.
        if pool.status == PoolStatus::Filling {
...
pub struct Withdraw<'info> {
    ...
    /// Always required in account list since Anchor validates all accounts.
    /// Transfer only occurs if pool_closed and rollover_seed > 0.
    #[account(
        mut,
